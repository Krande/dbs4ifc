...
using static ifc.Log;

public class ifcAttribute : NetSystem.Attribute
{
    public ifcAttribute(int OrdinalPosition, bool optional = false, bool derived = false)
    {
...
public class ifcInverseAttribute : NetSystem.Attribute
{
    public ifcInverseAttribute(string For)
    {
        this.For = For;
    }
...
// static:
    public partial class ENTITY : ifcSqlType
    {
        ...
        {
            LocalId = NextGlobalId++; /*NetSystem.Console.WriteLine(this.ToStepLine());*/
            Initialise();
            Repository.CurrentModel.EntityList.Add(this);
        }
        ...
        protected void AddNextCommentLine()
        {
            LocalId = NextGlobalCommentId--;
            Repository.CurrentModel.EntityList.Add(this);
        }
    } //=====================================================================================================================


    [ifcSql(TypeGroupId: 5, TypeId: -1, TableId: 3, LayerId: 0)]
    public partial class EntityComment : ENTITY
    {
        ...
        public EntityComment(string CommentLine)
        {
        ...
        public EntityComment(string CommentLine, char FrameChar)
        {
            this.Highlighted = Highlighting;
            this.CommentLine = CommentLine;
            if (this.CommentLine.Length < 74) this.CommentLine += new string(' ', 74 - this.CommentLine.Length);
    ...
    [ifcSql(TypeGroupId: 5, TypeId: -3, TableId: 3, LayerId: 0)]
    public partial class ImageComment : EntityComment
    {
        //==========================================================================================
        public ImageComment()
        {
        ...
        public ImageComment(string ImgFileName, int height = 0, int width = 0)
        {
            AddNextCommentLine();
            string TargetFileName = "copyof_" + global::System.IO.Path.GetFileName(ImgFileName);
            //this.CommentLine="<a href=\""+TargetFileName+"\">"+TargetFileName+"</a><BR/>"; // 2022-10-16 (bb) removed simple link, added www-refs
            this.CommentLine = "<a href=\"" + TargetFileName + "\" target=\"popup\" onclick=\"window.open('" +
                               TargetFileName + "','popup','width=400,height=400,toolbar=0'); return false;\">" +
                               ImgFileName + "</a>"; // (bb) added popup window for image-comments
            if (!File.Exists(TargetFileName)) // insert and copy only first time if file not exist
            {
                if (ImgFileName.Contains("http"))
                    try
                    {
                        new WebClient().DownloadFile(ImgFileName, TargetFileName);
                    }
                    catch (NetSystem.Exception e)
                    {
                        Add($"ImageComment:{ImgFileName} not found", Level.Exception);
                    }
                else
                {
                    if (File.Exists(ImgFileName)) File.Copy(ImgFileName, TargetFileName);
                    else Add($"ImageComment:{ImgFileName} not found", Level.Exception);
                }
    ...
    public partial class CartesianTransformationOperator3DnonUniform : CartesianTransformationOperator3D
    {
        ...
        {
            if (Scale == null) Scale = new Real(1);
            if (Scale2 == null) Scale2 = Scale;
            if (Scale3 == null) Scale3 = Scale;
            if (Axis1 == null) Axis1 = new Direction(x: 1, y: 0, z: 0);
            if (Axis2 == null) Axis2 = new Direction(x: 0, y: 1, z: 0);
            if (Axis3 == null) Axis3 = new Direction(x: 0, y: 0, z: 1);
        }
    } //=====================================================================================================================
#endif
} // ifc=======================================